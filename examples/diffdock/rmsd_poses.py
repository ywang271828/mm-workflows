...
if args.predicted_poses is not None:
    ...
    poses = [pose.replace('.sdf', '.mol') for pose in predicted_poses]
    ligmol = openbabel.OBMol()
    for i, pred_pose in enumerate(predicted_poses):
        molname = poses[i]
        obConversion.ReadFile(ligmol, pred_pose)
        obConversion.WriteFile(ligmol, molname)
    pred_mols = [Chem.MolFromMolFile(pose) for pose in poses]
...
def parse_confidence(file_name: str) -> float:
    ...
    Filenames most be of form rankX_confidenceY.mol,
    where X is a positive integer and Y is a float.
    This is default from diffdock outputs.
    ...
    """
    confidence = float(re.findall('rank[0-9]+_confidence(.*).mol', file_name)[0])
    return confidence
...
if SKIP is False:
    ...
    for cluster in clusters:
        names = [index_to_name[cluster_index] for cluster_index in cluster]
    ...
    pred_mols = [Chem.MolFromMolFile(pose) for pose in true_poses]

    # save pose name, RMSD filename to json file
    output_dict: Dict[str, Dict[str, float]] = {}
    for i, pred_name in enumerate(true_poses):
